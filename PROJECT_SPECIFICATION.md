...
- UI/runtime label: `Numberer B24 v.3.17`
- Marketplace zip: `dist app B24 zip/Numberer B24 v.3.17.zip`
...
- При сохранении администратором создается ревизия нумерации: обрабатываются только подходящие сделки с пустым полем `Уникальный номер`.
- Сделки, где `Уникальный номер` уже заполнен, сохраняют прежнее значение и не очищаются.
- Фоновый worker продолжает активную ревизию нумерации для подходящих сделок без номера и отдельно обрабатывает новые подходящие сделки без номера.
